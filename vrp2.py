...
# Ricerca sequenziale del NN di tutti i driver con criterio di scelta euristico
...
def check_dist(driver, nn, polo, j):
    if j == 3:
        if distanza(driver, polo) < distanza(nn, polo) and distanza(driver, nn) < distanza(nn, polo):
            return True
        else: 
            return False
    else:
    ...
    for i in range(200):
        ...
        for i in range(len(drivers_c)):
            ...
            for j in range(4): # Itero per ogni passeggero
                ...
                if check_dist(drivers_c[i], nn, polo, j):
                    tracks[i].append(nn)
                    users_c.remove(nn)
                    drivers_c[i] = nn
