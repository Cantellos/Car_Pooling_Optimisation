...
def msls(users, drivers, polo, base, n):
    ...
    for i in range(n):
        # Per ogni iterazione, si parte da una soluzione costruttiva diversa randomizzata
        drivers_r = drivers.copy()
   
        random.shuffle(drivers_r)
        print("R: ",drivers_r)
        print("Normal: ",drivers)

        # Scegli da quale euristica costruttiva iniziare
        if base == "greedy":
            tracks = greedy2(users.copy(), drivers_r, polo.copy())
        elif base == "kmeans":
            tracks = kmeans2(users.copy(), drivers_r, polo.copy())
        print(i)
        ...
        # Se la soluzione trovata è migliore della migliore soluzione trovata finora, aggiorna la migliore soluzione
        if funzione_obiettivo(tracks_c) <= best_msls_fo:
            best_msls_fo = funzione_obiettivo(tracks_c)
            best_msls_tracks = tracks_c.copy()
